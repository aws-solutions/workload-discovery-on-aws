...
const {AWS_LAMBDA_FUNCTION, AWS_EC2_VPC, AWS_EC2_INSTANCE, AWS_IAM_ROLE, AWS_RDS_DB_CLUSTER} = require("../../src/lib/constants");
...
describe('index.js', () => {
    ...
    describe('write errors', () => {
        it('returns delete failures', async () => {
            const mockApiClient = {
                deleteResources: sinon.stub().resolves(
                    {errors: [{item: ['arn1', 'arn2']}, {item: ['arn3']}, {item: ['arn4']}]}
                ),
                updateResources: sinon.stub().resolves(mockNoErrors),
                storeResources: sinon.stub().resolves(mockNoErrors),
                deleteRelationships: sinon.stub().resolves(mockNoErrors),
            ...
            const {failedDeletes} = await persistResourcesAndRelationships(mockApiClient, {
                resourceIdsToDelete: [], resourcesToStore: [], resourcesToUpdate: [],
                linksToAdd: [], linksToDelete: []
            });
        ...
        it('returns store failures', async () => {
            const mockApiClient = {
                deleteResources: sinon.stub().resolves(mockNoErrors),
                updateResources: sinon.stub().resolves(mockNoErrors),
                storeResources: sinon.stub().resolves(
                    {errors: [{item: [{id: 'arn1'}, {id: 'arn2'}]}, {item: [{id: 'arn3'}]}, {item: [{id: 'arn4'}]}]}
                ),
                deleteRelationships: sinon.stub().resolves(mockNoErrors),
                storeRelationships: sinon.stub().resolves(mockNoErrors),
            };

            const {failedStores} = await persistResourcesAndRelationships(mockApiClient, {
                resourceIdsToDelete: [], resourcesToStore: [], resourcesToUpdate: [],
                linksToAdd: [], linksToDelete: []
            });
    ...
    describe('processPersistenceFailures', () => {
        ...
        it('should removed failed stores', () => {
            const dbResources = [
                generateBaseResource(ACCOUNT_IDX, EU_WEST_1, AWS_LAMBDA_FUNCTION, 1),
                generateBaseResource(ACCOUNT_IDX, EU_WEST_1, AWS_EC2_VPC, 2),
                generateBaseResource(ACCOUNT_IDX, EU_WEST_1, AWS_EC2_INSTANCE, 3),
            ];

            const dbResourcesMap = new Map(dbResources.map(x => [x.id, x]));
            const resourceToStore = generateBaseResource(ACCOUNT_IDX, EU_WEST_1, AWS_IAM_ROLE, 4);
            const resourceToStoreFail = generateBaseResource(ACCOUNT_IDX, EU_WEST_1, AWS_RDS_DB_CLUSTER, 5);
            ...
            const actual = processPersistenceFailures(dbResourcesMap, resources, {
                failedDeletes: [], failedStores: [resourceToStoreFail.id]
            });
        ...
        it('should keep failed deletes', () => {
            ...
            const resourceToDelete = generateBaseResource(ACCOUNT_IDX, EU_WEST_1, AWS_IAM_ROLE, 4);
            const resourceToDeleteFail = generateBaseResource(ACCOUNT_IDX, EU_WEST_1, AWS_RDS_DB_CLUSTER, 5);
            ...
            const actual = processPersistenceFailures(dbResourcesMap, resources, {
                failedDeletes: [resourceToDeleteFail.id], failedStores: []
            });
