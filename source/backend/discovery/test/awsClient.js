...
const {OPENSEARCH} = require("../src/lib/constants");
...
describe('awsClient', () => {
        ...
        describe('getAllAggregatorResources', () => {
                ...
                mockConfigClient
                    ...
                    .callsFake(({ConfigurationAggregatorName, Expression, NextToken}) => {
                        const expectedExpression =
                            'SELECT *, configuration, configurationItemStatus, relationships, tags';
            ...
            it('should filter by resource type when getting resources from Config aggregator', async () => {
                ...
                mockConfigClient
                    ...
                    .callsFake(({ConfigurationAggregatorName, Expression, NextToken}) => {
                        const expectedExpression =
                            `SELECT *, configuration, configurationItemStatus, relationships, tags WHERE resourceType NOT IN ('AWS::RDS:DbInstance','AWS::EC2::VPC')`;
    ...
    describe('cognitoClient', () => {
        const {
            getAllUserPools
        } = awsClient.createCognitoClient(mockCredentials, EU_WEST_1);

        describe('getAllUserPools', () => {

            it('should get all user pools', async () => {
                const mockCognitoClient = mockClient(CognitoIdentityProviderClient);

                const userPoolsList = {
                    firstPage: {
                        UserPools: [
                            {Id: 'eu-west-1_aaaaaaaaa'},
                            {Id: 'eu-west-1_bbbbbbbbb'}
                        ],
                        NextToken: 'userPoolsListToken'
                    },
                    userPoolsListToken: {
                        UserPools: [
                            {Id: 'eu-west-1_ccccccccc'},
                            {Id: 'eu-west-1_ddddddddd'}
                        ]
                    }
                }

                const userPools = {
                    'eu-west-1_aaaaaaaaa': {Id: 'eu-west-1_aaaaaaaaa', Name: 'UserPoolA'},
                    'eu-west-1_bbbbbbbbb': {Id: 'eu-west-1_bbbbbbbbb', Name: 'UserPoolB'},
                    'eu-west-1_ccccccccc': {Id: 'eu-west-1_ccccccccc', Name: 'UserPoolC'},
                    'eu-west-1_ddddddddd': {Id: 'eu-west-1_ddddddddd', Name: 'UserPoolD'}
                };

                mockCognitoClient
                    .on(ListUserPoolsCommand)
                    .callsFake(({NextToken}) => {
                        if(NextToken != null) return userPoolsList[NextToken];
                        return userPoolsList.firstPage;
                    });

                mockCognitoClient
                    .on(DescribeUserPoolCommand)
                    .callsFake(({UserPoolId}) => {
                        return {
                            UserPool: userPools[UserPoolId]
                        }
                    });

                const actual = await getAllUserPools();
