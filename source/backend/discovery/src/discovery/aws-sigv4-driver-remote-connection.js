...
class AwsSigV4DriverRemoteConnection extends gremlin.driver.RemoteConnection {
  ...
  async callUrl(url) {
    const opts = getUrlAndHeaders(this.host, this.port, this.options, '/status', 'https');
    if(this.options.agent) {
  ...
  async connect() {
    ...
    try {
      ...
      logger.error("Error connecting to neptune: ", error);
      ...
      if (this.try <= this.maxRetry) {
        logger.error("Connection retry:" + this.try);
  ...
  _closeHandler(code, message) {
    logger.info('connection event: close', { code, message });
    this.connected = false;
  ...
  _errorHandler(error) {
    logger.error('connection event: error', { error });
    this.connected = false;
  ...
  async submit(bytecode) {
    ...
    if (this.connected) {
      ...
      if (this.submitTry <= this.maxSubmitTry) {
        await this.asleep(100 * this.submitTry);
