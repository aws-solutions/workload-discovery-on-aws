...
function createIndividualHandlers(lookUpMaps, awsClient) {
    ...
    return {
        ...
        [AWS_EVENT_RULE]: async ({configuration: {Targets}, relationships}) => {
            relationships.push(...Targets.flatMap(({Arn, RoleArn, EcsParameters}) => {
                const targetRelationship = createArnRelationship(IS_ASSOCIATED_WITH, Arn);
                const roleRelationship = createArnRelationship(IS_ASSOCIATED_WITH, RoleArn);
                if(EcsParameters != null) {
                    return [
                        targetRelationship,
                        roleRelationship,
                        createArnRelationship(IS_ASSOCIATED_WITH, EcsParameters.TaskDefinitionArn)
                    ];
                }
                return [targetRelationship, roleRelationship];
            }));
        },
