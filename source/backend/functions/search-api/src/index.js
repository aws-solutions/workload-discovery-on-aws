...
function unprocessedResourcesHandler(key, items) {
    return items.reduce((acc, {[key]: {error, _id}}) => {
        if (error != null) {
            console.log('Error writing item to index: ' + JSON.stringify(error));
            acc.push(_id);
        }
        return acc;
    }, []);
...
async function indexResources(osClient, resources) {
    const body = resources.flatMap(doc => {
        return [{index: {_index: INDEX, _type: docType, _id: doc.id}}, doc];
    });

    const {body: {errors, items}} = await osClient.bulk({body});

    const unprocessedResources = errors === false ? [] : unprocessedResourcesHandler('index', items);
...
async function updateResources(osClient, resources) {
    const body = resources.flatMap(doc => {
        return [{update: {_index: INDEX, _type: docType, _id: doc.id}}, {doc}];
    });

    const {body: {errors, items}} = await osClient.bulk({body});

    const unprocessedResources = errors === false ? [] : unprocessedResourcesHandler('update', items);
...
async function deleteIndexedResources(osClient, resourceIds) {
    const body = resourceIds.map(id => {
        return {delete: {_index: INDEX, _type: docType, _id: id}};
    });

    const {body: {errors, items}} = await osClient.bulk({body});

    const unprocessedResources = errors === false ? [] : unprocessedResourcesHandler('delete', items);
...
async function searchResources(osClient, text, {start = 0, end = 25}, accounts, resourceTypes) {
    const accountsBoolQuery = accounts.map(({accountId, regions}) => {
        const regionQuery = R.isNil(regions) ? [] : [{
            terms: {
                'properties.awsRegion.keyword': regions.map(x => x.name)
            }
        }]
    ...
    const resourceTypeQuery = R.isEmpty(resourceTypes) ? [] : [{terms: {'properties.resourceType.keyword': resourceTypes}}];

    return osClient.search({
        index: INDEX,
        type: docType,
        from: start,
        size: end - start,
    ...
    return async event => {
        ...
        switch (event.info.fieldName) {
            ...
            case 'searchResources':
                ...
                return searchResources(osClient, args.text, pagination, accounts, resourceTypes);
...
exports.handler = handler(osClient);
