...
import {privateLevel, useObject, usePutObject, viewsPrefix} from "../../../Hooks/useS3Objects";
...
const ViewFormPage = () => {
  ...
  const {data: selectedView} = useObject(name, viewsPrefix, privateLevel, {enabled: isEdit});
  ...
  useEffect(() => {
    if (selectedView) {
      setSelectedAccounts(R.pluck('accountId', R.pathOr([], ['accounts'], selectedView)));
