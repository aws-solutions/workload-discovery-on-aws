...
import { Alert, Button, SpaceBetween } from '@awsui/components-react';
...
const AccountImportDeploy = ({ regions }) => {
  ...
  const logVisit = (e) => {
    ...
    return window.open(generateCloudFormationLink(e.accountId, e.regionId), 'blank', 'rel=noreferrer');
  };
  ...
  return (
    <SpaceBetween direction='vertical' size='l'>
      <Alert
        ...
        We removed the Regions that are already discoverable to Perspective.
      </Alert>
      <AccountImportTable
        trackBy='id'
        rows={getRows()}
