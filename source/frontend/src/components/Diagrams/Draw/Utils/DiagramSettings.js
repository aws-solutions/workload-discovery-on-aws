...
const DiagramSettings = ({onSettingsChange, initialSettings}) => {
    ...
    const [lastSubmittedSettings, setLastSubmittedSettings] =
        useState(settings);
    const [hasDestructiveChanges, setHasDestructiveChanges] = useState(false);
    const [hasUnsavedChanges, setHasUnsavedChanges] = useState(false);
    const {data = {accounts: [], resourceTypes: []}, status} =
    ...
    const handleSubmit = () => {
        const isDestructiveKey = (val, key) =>
            !['costInterval', 'hideEdges'].includes(key);
    ...
    return (
        <form onSubmit={e => e.preventDefault()}>
                ...
                footer={
                    <Box float="right">
                        <SpaceBetween direction="horizontal" size="xs">
                            ...
                            <Button onClick={confirmSubmit} variant="primary">
                                Apply
                            </Button>
                        </SpaceBetween>
                    </Box>
            ...
            >
                <SpaceBetween size={'s'}>
                    <Box>
                        Modifying the filters will cause some elements to be
                        removed from your diagram. Are you sure you wish to
                        apply these settings?
                    </Box>
                    <Alert header="Persisting Settings">
                        You must also save the diagram after applying new
                        settings in order to persist these changes
                    </Alert>
                </SpaceBetween>
            </Modal>
            <Form
                ...
                <SpaceBetween size={'s'}>
                    <CostDatePicker
                        ...
                        onIntervalChange={val =>
                            handleChange('costInterval', val)
                        }
                    />
                    <FormField label={'Filter type'}>
                        <RadioGroup
                            onChange={({detail}) =>
                                handleChange('hideSelected', detail.value)
                            }
                    ...
                    >
                        <Multiselect
                            ...
                            filteringType={'auto'}
                            selectedOptions={options.accounts.filter(i =>
                                settings.accounts.includes(i.value)
                            )}
                    ...
                    >
                        <Multiselect
                            ...
                            filteringType={'auto'}
                            selectedOptions={options.regions.filter(i =>
                                settings.regions.includes(i.value)
                            )}
                            selectedAriaLabel="Selected"
                            expandToViewport
                            statusType={status}
                    ...
                    >
                        <Multiselect
                            ...
                            filteringType={'auto'}
                            selectedOptions={options.resourceTypes.filter(i =>
                                settings.resourceTypes.includes(i.value)
                            )}
                            selectedAriaLabel="Selected"
                            expandToViewport
                            statusType={status}
