...
import {Container, ExpandableSection, Header, SpaceBetween} from '@awsui/components-react';
...
const OpenDiagramPage = () => {
  ...
  useEffect(() => {
    if (canvas && settings) {
      ...
      const inSettingsArray = r => relevantProps
        .every(prop => {
          return (settings[prop] ?? []).includes(r.data("properties")[props[prop]])
        })
      const toRemove = settings?.hideSelected ?? true
        ? resources.filter(r => r.data("properties") && inSettingsArray(r))
        : resources.filter(r => r.data("properties") && !inSettingsArray(r))
  ...
  return (
    <SpaceBetween size='l'>
      ...
      />
      <Container header={<Header variant='h2' actions={<DiagramControlPanel settings={settings} />}>
        {name}
      </Header>}>
        <SpaceBetween size={"l"}>
          <ResourceSearch />
          ...
          />
          {
            settings && (
              <ExpandableSection header={"Diagram Settings"}>
                <DiagramSettings initialSettings={settings} onSettingsChange={setSettings}/>
              </ExpandableSection>
            )
          }
        </SpaceBetween>
