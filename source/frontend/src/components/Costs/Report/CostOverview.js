    ...
    R.forEach((n) => {
      if (R.hasPath(['data', 'resourceId'], n)) {
        if (R.includes(e.id, n.data.resourceId)) {
          processedResources.push(R.mergeDeepRight(n, {
            data: {
              cost: e.cost,
              service: e.service
            }
...
const CostOverview = () => {
  ...
  const {data: diagramData=[], isLoading: loadingDiagram} = useObject(name, diagramsPrefix, visibility);
  const resources = R.compose(
    R.uniqBy(R.pathEq(['data', 'title'])),
    R.filter((e) => R.equals(e.data?.type, 'resource'))
  )(diagramData.nodes ?? {})
  const {data: costs=[], isLoading: loadingCosts} = useResourceCosts(resources, dateInterval);
  const costResources = processCosts(costs, resources)
  const { data: resourceDailyBreakdown=[], isLoading: loadingDailyBreakdown} = useDailyResourceCosts(selectedItems, dateInterval, getPageSize(selectedItems, dateInterval))
    ...
    <SpaceBetween direction='vertical' size='l'>
      <Breadcrumbs
        items={[
          ...
          { text: name, href: OPEN_DIAGRAM.replace(':name', name).replace(
              ':visibility',
              visibility
            ) },
