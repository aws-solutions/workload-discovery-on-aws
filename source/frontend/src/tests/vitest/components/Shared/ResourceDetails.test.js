...
describe('Resource Details', () => {
    it('should display resource details', async () => {
        ...
        await user.click(subnet0Checkbox);
        ...
        await screen.findByRole('link', {
            name: /arn:aws:xxxxxxxxxxxx:eu-west-2:aws::ec2::subnet:0/i,
        });
        ...
        const resourcesDetailsTableWrapper = new TableWrapper(
            resourcesDetailsTable.parentElement
        );

        expect(getCellText(1, 1, resourcesDetailsTableWrapper)).toMatch(
            /Resource type/
        );
        expect(getCellText(2, 1, resourcesDetailsTableWrapper)).toMatch(
            /Account Id/
        );
        expect(getCellText(3, 1, resourcesDetailsTableWrapper)).toMatch(
            /Region/
        );
        expect(getCellText(4, 1, resourcesDetailsTableWrapper)).toMatch(
            /Availability zone/
        );

        expect(getCellText(1, 2, resourcesDetailsTableWrapper)).toBe(
            'AWS::EC2::Subnet'
        );
        expect(getCellText(2, 2, resourcesDetailsTableWrapper)).toBe(
            'xxxxxxxxxxxx'
        );
        expect(getCellText(3, 2, resourcesDetailsTableWrapper)).toBe(
            'eu-west-2'
        );
        expect(getCellText(4, 2, resourcesDetailsTableWrapper)).toBe(
            'availabilityZone'
        );

        expect(getCellText(1, 3, resourcesDetailsTableWrapper)).toBe(
            'AWS::EC2::Subnet'
        );
        expect(getCellText(2, 3, resourcesDetailsTableWrapper)).toBe(
            'xxxxxxxxxxxx'
        );
        expect(getCellText(3, 3, resourcesDetailsTableWrapper)).toBe(
            'eu-west-2'
        );
        expect(getCellText(4, 3, resourcesDetailsTableWrapper)).toBe(
            'availabilityZone'
        );
    });
});
