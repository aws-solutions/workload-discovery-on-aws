...
describe('View Page', () => {
    ...
    it('should create new view', async () => {
        ...
        const viewsLink = screen.getByRole('link', { name: /Views$/, hidden: true });
        await userEvent.click(viewsLink);

        await userEvent.click(await screen.findByRole('button', { name: /create/i }));

        const accountFilterButton = screen.getByRole('button', { name: /accounts choose accounts to filter by/i })
        await userEvent.click(accountFilterButton);

        const filterOption = await screen.findByRole('option', { name: /xxxxxxxxxxxx/i });
        ...
        await screen.findByRole('heading', {level: 2, name: /Resources types \(6\)/});

        const subnetCheckbox = screen.getByRole('checkbox', {
            name: /aws::ec2::subnet is not selected/i
        });

        await userEvent.click(subnetCheckbox);

        const saveButton = screen.getByRole('button', { name: /save/i });
        await userEvent.click(saveButton);

        await screen.findByRole('heading', {level: 2, name: /Resources \(2\)/}, {timeout: 2000});
        ...
        const resourcesTypeTableWrapper = new TableWrapper(resourcesTypeTable.parentElement);
        const resourcesTypeTableRows = resourcesTypeTableWrapper.findRows();

        expect(resourcesTypeTableRows).toHaveLength(1);

        expect(getCellText(resourcesTypeTableWrapper, 1, 1)).toMatch(/\/icons\/VPC-subnet-private_light-bg-menu.svg/);
        expect(getCellText(resourcesTypeTableWrapper, 1, 2)).toMatch(/AWS::EC2::Subnet/);
        ...
        expect(getCellText(resourcesTableWrapper, 1, 2)).toMatch(/\/icons\/VPC-subnet-private_light-bg-menu.svg/);
        expect(getCellText(resourcesTableWrapper, 1, 3)).toBe('arn:aws:xxxxxxxxxxxx:eu-west-2:AWS::EC2::Subnet:0Title');
        expect(getCellText(resourcesTableWrapper, 1, 4)).toBe('AWS::EC2::Subnet');
        ...
        expect(getCellText(resourcesTableWrapper, 2, 2)).toMatch(/\/icons\/VPC-subnet-private_light-bg-menu.svg/);
        expect(getCellText(resourcesTableWrapper, 2, 3)).toBe('arn:aws:xxxxxxxxxxxx:eu-west-2:AWS::EC2::Subnet:1Title');
        expect(getCellText(resourcesTableWrapper, 2, 4)).toBe('AWS::EC2::Subnet');
