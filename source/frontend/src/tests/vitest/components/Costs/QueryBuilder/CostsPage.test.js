...
function verifyTableRow(table, rowNumber, index) {
    ...
    expect(table.findBodyCell(rowNumber, 5).getElement().innerHTML).toBe(
        costInfo['region']
    );
}
...
describe('Costs Page', () => {
    describe('Costs Overview', () => {
        ...
        it(
            'should display cost by resource data',
            async () => {
                const user = userEvent.setup();

                renderCostsPage();

                const serviceRadioButton = screen.getByRole('radio', {
                    name: /query by arn or resource id/i,
                });

                await user.click(serviceRadioButton);
                ...
                await user.type(
                    resourceInput,
                    resourceData['costItems'][0]['line_item_resource_id']
                );
        ...
        it('should display cost by service data', async () => {
            ...
            const serviceRadioButton = await screen.getByRole('radio', {
                name: /query by service/i,
            });

            await user.click(serviceRadioButton);

            const serviceSelectorButton = await screen.getByRole('button', {
                name: /service name select a service/i,
            });

            await user.click(serviceSelectorButton);

            const serviceOption = screen.getByRole('option', {
                name: /cloudtrail/i,
            });
